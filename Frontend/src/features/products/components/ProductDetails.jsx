                ...
                <Stack width={is480?"auto":is1420?"auto":'88rem'} p={is480?2:0} height={is840?"auto":"50rem"} rowGap={5} mt={is840?0:5} justifyContent={'center'} mb={5} flexDirection={is840?"column":"row"} columnGap={is990?"2rem":"5rem"}>
                    ...
                    <Stack rowGap={"1.5rem"} width={is480?"100%":'25rem'}>
                        ...
                        <Stack sx={{rowGap:"1.3rem"}} width={'fit-content'}>
                            ...
                            {/* Medical Information */}
                            <Stack sx={{mt:2, rowGap:"0.8rem"}}>
                                {product?.requiresPrescription && <Typography color="error" fontWeight={600}>Rx Prescription Required</Typography>}
                                {product?.manufacturer && <Typography><strong>Manufacturer:</strong> {product?.manufacturer}</Typography>}
                                {product?.dosage && <Typography><strong>Dosage:</strong> {product?.dosage}</Typography>}
                                {product?.medicineType && <Typography><strong>Medicine Type:</strong> {product?.medicineType}</Typography>}
                                {product?.expiryDate && <Typography><strong>Expiry Date:</strong> {new Date(product?.expiryDate).toLocaleDateString()}</Typography>}
                            </Stack>
